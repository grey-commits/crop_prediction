...
def get_user_input(numerical_features):
    ...
    for feature in numerical_features:
        ...
        while True:
            try:
                unit = "°C" if feature == "temperature" else ("%" if feature == "humidity" else ("mm" if feature == "rainfall" else ""))
                user_input[feature] = float(input(f"Enter {feature} ({min_val:.2f} - {max_val:.2f} {unit}): "))
                if min_val <= user_input[feature] <= max_val:
                    break
                else:
...
def main():
    ...
    file_path = os.path.join(script_dir, 'crop_recommendation_cleaned.csv')
    model_filename = os.path.join(script_dir, 'crop_recommendation_model.joblib')
    scaler_filename = os.path.join(script_dir, 'crop_recommendation_scaler.joblib')
    label_encoder_filename = os.path.join(script_dir, 'crop_recommendation_label_encoder.joblib')
